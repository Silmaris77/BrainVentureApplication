...
class ThemeProvider:
    ...
    def initialize():
        ...
        # Add JavaScript for checking localStorage
        st.markdown(
            """
            <script>
            // Function to set Streamlit session state from localStorage
            function setupTheme() {
                const storedTheme = localStorage.getItem('brainventure_ui_theme');
                if (storedTheme) {
                    console.log("Found stored theme: " + storedTheme);
                    // The theme will be applied through Python logic
                }
            }
        ...
        with st.sidebar:
            ...
            for idx, theme in enumerate(UITheme):
                with cols[idx]:
                    ...
                    btn_key = f"layout_btn_{theme.name.lower()}"
                    
                    # Set button type based on current theme
                    btn_type = "primary" if st.session_state.ui_theme == theme else "secondary"
